...
Deno.serve(async (req) => {
  try {
    ...
    const leaders = Array.from(agg.entries()).map(([playerId, { total, games }]) => {
      ...
      return { player_id: playerId, first_name: player.first_name, last_name: player.last_name, jersey_number: player.jersey_number || '', team_id: player.team_id, team_name: team.name || 'Unknown', team_logo_url: team.logo_url || '', total, games_played: gamesPlayed, average: gamesPlayed > 0 ? Number((total / gamesPlayed).toFixed(1)) : 0, average_label: 'RPG', photo_url: player.photo_url || '' };
    ...
    console.error('getTopRebounders error:', error?.message || error);
