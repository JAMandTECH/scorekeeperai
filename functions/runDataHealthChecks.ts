...
// Admin-only data health checks over current admin's organization
// Scans for referential integrity, sport mismatches, and score/stat inconsistencies (basketball & volleyball)
// Usage (frontend): await base44.functions.invoke('runDataHealthChecks', {})
// Response: { ok: true, organization_id, summary, issues: { ...detail arrays (capped) } }
...
Deno.serve(async (req) => {
  try {
    ...
    if (!orgId) {
      return Response.json({ error: 'No organization context found for user' }, { status: 400 });
    }

    // Helpers
    const cap = (arr, n = 50) => ({ items: arr.slice(0, n), more_count: Math.max(arr.length - n, 0) });

    // Load core entities scoped to the admin's organization
    const teams = await base44.asServiceRole.entities.Team.filter({ organization_id: orgId });

    // Players by team (parallel per team to stay within limits)
    const teamIds = teams.map(t => t.id);
    const playersNested = await Promise.all(
      teamIds.map((id) => base44.asServiceRole.entities.Player.filter({ team_id: id }).catch(() => []))
    );
    ...
    // PlayerGameStats for relevant games (chunk by 50 ids)
    ...
    for (let i = 0; i < gameIds.length; i += 50) {
      ...
      try {
        ...
        // Fallback: one-by-one to avoid losing the whole chunk
    ...
    // Referential integrity & value checks in stats
    for (const s of stats) {
      ...
      // Non-negative basic stat fields
      const numericKeys = [
      ...
      for (const k of numericKeys) {
        ...
        if (Number.isNaN(v) || v < 0) {
          invalidValues.push({ type: 'invalid_stat_value', key: k, value: s[k], stat_id: s.id });
        }
      }

      // Sport alignment checks
      if (g && t) {
        if (g.sport !== t.sport) {
        ...
        if (g.sport === 'volleyball') {
          // Basketball-only fields should be 0 in volleyball
        ...
        if (g.sport === 'basketball') {
          // Volleyball-only fields should be 0 in basketball
    ...
    for (const g of games) {
      if (g.home_timeouts != null && (g.home_timeouts < 0 || g.home_timeouts > 5)) {
        invalidGameFields.push({ type: 'invalid_home_timeouts', game_id: g.id, value: g.home_timeouts });
      }
      if (g.away_timeouts != null && (g.away_timeouts < 0 || g.away_timeouts > 5)) {
        invalidGameFields.push({ type: 'invalid_away_timeouts', game_id: g.id, value: g.away_timeouts });
      }
    ...
    // Score consistency (completed games only)
    const completed = games.filter(g => g.status === 'completed');

    // Helper to compute points from stats, by game and team
    const computeTeamPoints = (game, teamId) => {
      ...
      if (game.sport === 'volleyball') {
        // Sum aces + attacks + blocks
        return related.reduce((sum, s) => sum + Number(s.aces||0) + Number(s.attacks||0) + Number(s.blocks||0), 0);
      } else {
        // Basketball: prefer stored points if present; otherwise compute from FGM/3PM/FTM
    ...
    for (const g of completed) {
      ...
      const mismatch = (homePts !== Number(g.home_score || 0)) || (awayPts !== Number(g.away_score || 0));
      if (mismatch) {
        scoreMismatches.push({
          game_id: g.id,
          sport: g.sport,
          expected_home: g.home_score || 0,
          expected_away: g.away_score || 0,
          computed_home: homePts,
          computed_away: awayPts,
        });
      }
    }
