...
const getDivisionStandings = async payload => {
  ...
  for (const game of completed) {
    ...
    const hs = num(game.home_score); const as = num(game.away_score);
    if (hs === as) continue;
  ...
  return {
    ...
    teams: filtered.slice(0, limit).map((t, idx) => { const gp = t.wins + t.losses; return { rank: idx + 1, team_id: t.id, name: t.name, division: t.division || '', wins: t.wins, losses: t.losses, win_pct: gp ? Number((t.wins / gp).toFixed(3)) : 0, logo_url: t.logo_url || null }; }),
...
const getTopAssistLeaders = async payload => {
  ...
  const playerMap = new Map(players.map(p => [p.id, p])); const totals = new Map();
...
const invokeLocal = async (name, payload = {}) => {
  if (name === 'updateGame') {
    const game = await entity('Game').get(payload.game_id); if (!game) throw new Error('Game not found');
    return { data: await entity('Game').update(payload.game_id, payload.patch || {}) };
  }
  if (name === 'getGamePlayerStats') {
    const ids = payload.game_ids || []; const rows = await entity('PlayerGameStats').filter({}, '-created_date'); return { data: rows.filter(r => ids.includes(r.game_id)) };
  }
  if (name === 'upsertPlayerStat') {
    const rows = await entity('PlayerGameStats').filter({ game_id: payload.game_id, player_id: payload.player_id }); const existing = rows.find(r => num(r.quarter) === num(payload.quarter));
  ...
  // All remaining server-side operations use the single ScorePilot Edge Function.
