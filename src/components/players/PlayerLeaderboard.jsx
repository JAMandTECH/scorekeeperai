...
export default function PlayerLeaderboard({
  ...
  return (
    <div className="rounded-lg bg-card border border-border overflow-hidden p-2 sm:p-3 space-y-2">
      {ranked.map((player, idx) => {
        ...
        return (
          <div
            key={player.id}
            className="group relative flex items-center gap-2 sm:gap-4 rounded-lg bg-muted/40 border border-border px-2 sm:px-4 py-2.5 hover:bg-muted transition-colors"
          >
            {/* Rank + movement */}
            <div className="flex items-center gap-1.5 w-11 sm:w-14 shrink-0">
              {isTop ? (
                <Trophy className="w-6 h-6 text-primary fill-primary/20 mx-auto" />
              ) : (
                <span className="text-2xl font-black text-foreground tabular-nums w-7 text-center">
                  {rank}
                </span>
              )}
            ...
            <div className="relative shrink-0">
              <Avatar className="w-11 h-11 sm:w-12 sm:h-12 border-2 border-border">
            ...
            <div className="min-w-0 flex-1">
              <p className="font-bold text-foreground uppercase tracking-wide text-sm sm:text-[15px] truncate">
            ...
            <div className="hidden md:flex items-center">
              {(player.stats || []).map((stat, i) => (
                <div
                  key={i}
                  className="text-center px-3 lg:px-3.5 border-l border-border first:border-l-0"
                >
            ...
            <div className="shrink-0 ml-1 sm:ml-2">
              <div className="bg-gradient-to-br from-rose-500 to-rose-700 text-white font-black text-xl sm:text-2xl rounded-lg px-3.5 sm:px-5 py-2 sm:py-2.5 min-w-[56px] sm:min-w-[68px] text-center shadow-lg shadow-rose-500/25 tabular-nums">
                {score}
              </div>
            </div>
              ...
              {canManagePlayers && (
                <>
                  <Button
                    ...
                    className="h-8 w-8 text-muted-foreground hover:text-foreground hover:bg-muted hidden sm:inline-flex"
                  >
                  ...
                  <Button
                    ...
                    className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-muted hidden sm:inline-flex"
                  >
